...
Access.prototype._submit = function (result) {
    ...
    var query = Query.createPostType(url, paramMap, function (data) {
        if (access.getType() == Access.LOGIN){
            //构建用户信息类，绑定到systemInfo上
            var userInfo = UserInfo.getUserInfo();
            userInfo.setUserName(data.userName);
            window.systemInfo.setUserInfo(userInfo);
        }
        window.tool.prompt(successMsg,window.tool.prompt_success);
        $('#accessModal').modal('hide');

    }, function (data) {
        window.tool.smratErrorPrompt(data);
        //重新生成验证码
...
Access.prototype._checkPasswordInput = function (password, passwordJQ, promptJQ) {
    ...
    if (window.tool.isLetterDigit(password)) {
